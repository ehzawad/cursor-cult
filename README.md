...
Read-only is therefore enforced by Cursor itself rather than inferred from `--mode`. Every non-writer role runs against its own Cursor configuration, supplied through `CURSOR_CONFIG_DIR`, carrying `permissions.deny` of `Write(**)` and `Shell(*)`. Cursor documents that deny rules take precedence over allow rules, and that holds even in agent mode with `--force`: a denied write and a denied shell command are both refused. The generated configuration preserves the operator's existing allow and deny entries and appends role restrictions; malformed permission arrays fail closed. Workers also pass `--disable-project-configs`, because Cursor's project-level `.cursor/cli.json` permission arrays otherwise replace the global arrays and could erase the role boundary.
