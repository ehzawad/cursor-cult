...
- Read-only is reinforced through Cursor mode when available and an explicit prompt contract, but it is not an OS sandbox.
- `--force` is granted only to the sole writer.
