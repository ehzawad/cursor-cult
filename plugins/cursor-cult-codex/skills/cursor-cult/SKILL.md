...
Add exactly one `--writer <role-id>` only when that role is authorized to edit this worktree. Do not impose an outer wall-clock timeout. Read both outputs; normal stderr ends with `CURSOR_CULT_DONE`.
