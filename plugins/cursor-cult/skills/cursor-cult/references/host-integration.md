...
For the packaged Claude Code plugin, `monitors/monitors.json` starts `watch-all` on the first `cursor-cult` skill invocation. On Claude Code v2.1.105 or later, in an interactive CLI session where the experimental Monitor facility is available, plugin monitors deliver every stdout line into the live session, so heartbeats and terminal completion reach the main harness without polling. Events carry a stable `(run_id, sequence)` identity so a restarted monitor can replay safely and the host can deduplicate. If plugin monitors are unavailable, the host must run the returned `watch_command` with Claude's Monitor tool; use background Bash only as a fallback and retain its task ID.
